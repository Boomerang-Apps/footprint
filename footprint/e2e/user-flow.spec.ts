import { test, expect } from '@playwright/test';
import path from 'path';
import fs from 'fs';

/**
 * E2E Test: Complete User Flow
 * Tests the full journey: Upload -> Style -> Customize -> Checkout
 *
 * Story: WAVE1-QA-001, AC-008
 *
 * Note: Some pages require session state (uploaded image, selected style).
 * These tests verify page accessibility and basic structure.
 */

// Test fixture paths
const FIXTURES_DIR = path.join(__dirname, 'fixtures');
const TEST_IMAGE_PATH = path.join(FIXTURES_DIR, 'test-image.jpg');

test.beforeAll(async () => {
  // Ensure fixtures directory exists
  if (!fs.existsSync(FIXTURES_DIR)) {
    fs.mkdirSync(FIXTURES_DIR, { recursive: true });
  }

  // Create a minimal valid JPEG if test image doesn't exist
  if (!fs.existsSync(TEST_IMAGE_PATH)) {
    // Minimal valid JPEG (1x1 red pixel)
    const minimalJpeg = Buffer.from([
      
    ]);
    fs.writeFileSync(TEST_IMAGE_PATH, minimalJpeg);
  }
});

test.describe('User Flow E2E', () => {

  test.describe('Page Navigation', () => {
    test('can access upload page', async ({ page }) => {
      const response = await page.goto('/create');
      expect(response?.status()).toBeLessThan(500);
    });

    test('can access style page', async ({ page }) => {
      const response = await page.goto('/create/style');
      expect(response?.status()).toBeLessThan(500);
    });

    test('can access customize page', async ({ page }) => {
      const response = await page.goto('/create/customize');
      expect(response?.status()).toBeLessThan(500);
    });

    test('can access checkout page', async ({ page }) => {
      const response = await page.goto('/create/checkout');
      expect(response?.status()).toBeLessThan(500);
    });

    test('can access complete page', async ({ page }) => {
      const response = await page.goto('/create/complete');
      expect(response?.status()).toBeLessThan(500);
    });

    test('navigates through all steps sequentially', async ({ page }) => {
      // Step 1: Start at upload
      await page.goto('/create');
      await page.waitForLoadState('domcontentloaded');

      // Step 2: Navigate to style
      await page.goto('/create/style');
      await page.waitForLoadState('domcontentloaded');

      // Step 3: Navigate to customize
      await page.goto('/create/customize');
      await page.waitForLoadState('domcontentloaded');

      // Step 4: Navigate to checkout
      await page.goto('/create/checkout');
      await page.waitForLoadState('domcontentloaded');

      // Step 5: Navigate to complete
      await page.goto('/create/complete');
      await page.waitForLoadState('domcontentloaded');

      // All pages should be accessible without 500 errors
      expect(true).toBe(true);
    });
  });

  test.describe('Upload Page', () => {
    test('page loads without server error', async ({ page }) => {
      const response = await page.goto('/create');
      expect(response?.status()).toBeLessThan(500);
      await page.waitForLoadState('domcontentloaded');
    });

    test('page renders without throwing', async ({ page }) => {
      await page.goto('/create');
      await page.waitForLoadState('networkidle');
      // Page should complete loading without errors
      const title = await page.title();
      expect(title).toBeDefined();
    });
  });

  test.describe('Style Page', () => {
    test('page loads without server error', async ({ page }) => {
      const response = await page.goto('/create/style');
      expect(response?.status()).toBeLessThan(500);
    });

    test('page renders without throwing', async ({ page }) => {
      await page.goto('/create/style');
      await page.waitForLoadState('networkidle');
      // Page should complete loading without errors
      const title = await page.title();
      expect(title).toBeDefined();
    });
  });

  test.describe('Checkout Page', () => {
    test('page loads without server error', async ({ page }) => {
      const response = await page.goto('/create/checkout');
      expect(response?.status()).toBeLessThan(500);
    });

    test('page renders without throwing', async ({ page }) => {
      await page.goto('/create/checkout');
      await page.waitForLoadState('networkidle');
      // Page should complete loading without errors
      const title = await page.title();
      expect(title).toBeDefined();
    });
  });

  test.describe('API Endpoints', () => {
    test('upload API returns appropriate response', async ({ page }) => {
      const response = await page.request.post('/api/upload', {
        headers: { 'Content-Type': 'application/json' },
        data: {}
      });

      // Should return 400 (bad request) or 401 (unauthorized), not 500
      expect(response.status()).toBeLessThan(500);
    });

    test('checkout API requires authentication', async ({ page }) => {
      const response = await page.request.post('/api/checkout', {
        headers: { 'Content-Type': 'application/json' },
        data: { items: [] }
      });

      // Should return 400 or 401, not 500
      expect(response.status()).toBeLessThan(500);
    });

    test('transform API returns appropriate response', async ({ page }) => {
      const response = await page.request.post('/api/transform', {
        headers: { 'Content-Type': 'application/json' },
        data: {}
      });

      // Should return 400 (bad request) or 401 (unauthorized), not 500
      expect(response.status()).toBeLessThan(500);
    });
  });

  test.describe('Error Handling', () => {
    test('404 page does not crash', async ({ page }) => {
      const response = await page.goto('/create/nonexistent-page-12345');
      // Should return 404, not 500
      expect(response?.status()).toBeLessThan(500);
    });

    test('invalid API request does not crash server', async ({ page }) => {
      const response = await page.request.post('/api/checkout', {
        headers: { 'Content-Type': 'application/json' },
        data: { invalid: 'data' }
      });

      // Should handle gracefully (4xx), not crash (5xx)
      expect(response.status()).toBeLessThan(500);
    });
  });

  test.describe('Security', () => {
    test('webhook endpoint rejects unsigned requests', async ({ page }) => {
      const response = await page.request.post('/api/webhooks/payplus', {
        headers: { 'Content-Type': 'application/json' },
        data: { fake: 'webhook' }
      });

      // Should reject (400/401/403), not accept
      expect(response.status()).toBeGreaterThanOrEqual(400);
      expect(response.status()).toBeLessThan(500);
    });

    test('admin API requires authentication', async ({ page }) => {
      const response = await page.request.get('/api/admin/orders');

      // Should return 401 or 403, not 200 or 500
      expect([401, 403, 404]).toContain(response.status());
    });
  });

  test.describe('Performance', () => {
    test('pages load within acceptable time', async ({ page }) => {
      const start = Date.now();
      await page.goto('/create');
      await page.waitForLoadState('domcontentloaded');
      const loadTime = Date.now() - start;

      // Should load in under 10 seconds
      expect(loadTime).toBeLessThan(10000);
    });
  });
});
