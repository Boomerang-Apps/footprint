...
# Create Agent Validation Protocol
echo "🔒 Creating validation protocol..."
cat > ".claudecode/workflows/AGENT-VALIDATION-PROTOCOL.md" << 'EOF'
...
2. Read safety framework
3. Read workflow protocol
4. Read validation protocol
5. Check inbox
6. Confirm branch/worktree
7. Output Identity Declaration
...
╠══════════════════════════════════════════════════════════════════╣
║  Agent: [Name] | Domain: [Domain] | Branch: [Branch]             ║
║  DOCUMENTS READ: ✅ All 5 required documents                     ║
║  BOUNDARIES: [What I do] / [What I never do]                     ║
║  ASSIGNMENT: [Story ID or Awaiting]                              ║
╚══════════════════════════════════════════════════════════════════╝
```
...
**IF ANY CHECK FAILS → STOP. ASK PM.**

---

## LAYER 3: CONTINUOUS VALIDATION

Every response MUST include Safety Banner.
Every 3 actions → Periodic Self-Check.

---

## LAYER 4: HANDOFF VALIDATION

- Complete work → qa-inbox.md
- Blocked → pm-inbox.md
- Security issue → cto-inbox.md
- NEVER hand off to other dev agents directly

---

## RED FLAGS - STOP IF:

- About to edit file outside domain
- No test exists for code being written
- No START.md exists
- No inbox assignment
- Handing off to another dev
- Coverage below 80%
...
# Create Agent Startup Protocol
cat > ".claudecode/AGENT-STARTUP-PROTOCOL.md" << 'EOF'
# MANDATORY AGENT STARTUP PROTOCOL

Before ANY work, agent MUST:

1. Read: .claudecode/agents/[my-agent]-agent.md
2. Read: .claudecode/workflows/MANDATORY-SAFETY-FRAMEWORK.md
3. Read: .claudecode/workflows/WORKFLOW-2.0-PM-ORCHESTRATION.md
4. Read: .claudecode/workflows/AGENT-VALIDATION-PROTOCOL.md
5. Read: .claudecode/handoffs/[my-agent]-inbox.md
6. Output Identity Declaration
7. Wait for confirmation before any work

**IF AGENT CANNOT COMPLETE → STOP. REPORT TO PM.**
EOF

echo "✅ Validation protocol created"
