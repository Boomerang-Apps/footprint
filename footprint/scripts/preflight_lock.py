...
CRITICAL_FILES = {
    ...
    "skills": [
        ...
        ".claude/commands/rlm.md",
        ".claude/commands/commit.md",
        ".claude/commands/story.md",
        ".claude/commands/pr.md",
        ".claude/commands/agent.md",
        ".claude/commands/wave-start.md",
        ".claude/commands/build.md",
        ".claude/commands/test.md",
        ".claude/commands/status.md",
        ".claude/commands/safety.md",
...
class PreFlightValidator:
    ...
    def validate_all(self) -> bool:
        ...
        checks = [
            ...
            ("Skills (Claude Commands)", self.check_skills),
    ...
    def check_skills(self) -> bool:
        """Verify all Claude Code skills exist with required structure."""
        skills_dir = PROJECT_ROOT / ".claude" / "commands"

        if not skills_dir.exists():
            self.errors.append("Skills directory missing: .claude/commands/")
            return False

        required_skills = CRITICAL_FILES.get("skills", [])
        missing_skills = []
        invalid_skills = []
        ...
        for skill_path in required_skills:
            ...
            if not full_path.exists():
                ...
                content = full_path.read_text()
                required_sections = ["Priority:", "Recommended Model:", "## Purpose"]
                for section in required_sections:
                    if section not in content:
                        invalid_skills.append(f"{skill_path} missing '{section}'")

                # Compute hash for lock file
                self.file_hashes[skill_path] = hashlib.sha256(
                    full_path.read_bytes()
                ).hexdigest()

        if missing_skills:
            self.errors.append(f"Missing skills: {missing_skills}")
            return False

        if invalid_skills:
            self.warnings.append(f"Skills with invalid structure: {invalid_skills}")
